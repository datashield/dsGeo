...
overMatchDS <- function(points, points_id, over_out, locations_id){
  
  size <- length(over_out)
  over_ind <- vector(mode="logical", length=size)
  for (i in 1:size){
    over_ind[i] <- (points[[points_id]][i] %in% over_out[[i]][[locations_id]] )*1
  }
